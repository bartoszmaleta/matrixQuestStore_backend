...
public class MentorHandler implements HttpHandler {
    ...
    public void handle(HttpExchange httpExchange) throws IOException {
        ...
        try {
            switch (method) {
                case "GET" -> response = handleAction(actions, action);
                case "POST" -> {
...
//                         response = this.mapper.writeValueAsString(awards);
    ...
    private boolean getDetailsAction(String action1) {
    ...
    private String handleAction(String[] actions, String action) throws JsonProcessingException, FileNotFoundException {
        switch (action) {
            case "details" -> {
